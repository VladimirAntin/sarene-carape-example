...
const TAGS = [
  ...
  {emoji: '🎁', label: 'Pokloni', color: 'bg-emerald-500/20 border-emerald-400/30 text-emerald-100'},
];

const FLOATERS = ['🧦', '🌈', '🎨', '🧶', '✨', '🎀'];

const Highlight: FC<{children?: ReactNode}> = ({children}) => (
  <span className={'bg-linear-to-r from-pink-300 via-yellow-300 to-purple-300 bg-clip-text text-transparent'}>
    {children}
  </span>
);
  ...
  return (
    <section className={'relative flex min-h-screen flex-col items-center justify-center overflow-hidden'}>
      ...
      <div className={'absolute inset-0'}>
        ...
        <div className={'absolute inset-0 bg-linear-to-br from-pink-950/90 via-purple-900/80 to-indigo-950/90'} />
        {/* Decorative rainbow blobs */}
        <div className={'absolute top-1/4 left-1/6 h-72 w-72 rounded-full bg-pink-500/20 blur-3xl'} />
        <div className={'absolute bottom-1/3 right-1/5 h-96 w-96 rounded-full bg-purple-500/15 blur-3xl'} />
        <div className={'absolute top-2/3 left-1/2 h-64 w-64 rounded-full bg-fun-orange/15 blur-3xl'} />
        <div className={'absolute top-1/3 right-1/3 h-48 w-48 rounded-full bg-fun-yellow/10 blur-3xl'} />
      </div>
      ...
      <div className={'relative z-10 mx-auto max-w-5xl px-4 py-20 text-center sm:px-6 lg:px-8'}>
        ...
        <motion.div
          ...
          <span className={'h-2 w-2 animate-pulse rounded-full bg-fun-green'} />
        ...
        <motion.div
          ...
          <Link
            href={'/proizvodi'}
            className={'group flex items-center gap-2 rounded-full bg-primary px-8 py-4 font-semibold text-white shadow-lg shadow-primary/40 transition hover:bg-primary-dark'}>
            {'🧦 Istraži Kolekciju'}
            <span className={'transition-transform duration-200 group-hover:translate-x-1'}>{'→'}</span>
          </Link>
          <Link
            href={'/#kolekcija'}
            className={'group flex items-center gap-2 rounded-full border border-white/40 bg-white/10 px-8 py-4 font-semibold text-white backdrop-blur-sm transition hover:border-white/70 hover:bg-white/20'}>
            {'🌈 Naše Kategorije'}
            <span className={'transition-transform duration-200 group-hover:translate-x-1'}>{'→'}</span>
          </Link>
        </motion.div>
      ...
      <motion.a
        ...
        <span className={'text-[10px] font-medium tracking-widest text-white/40 uppercase'}>{'Otkrij više'}</span>
      </motion.a>
